...
// auto record user preset history
const userPresetHistory = [];
...
const module = (name, initValues, methods) => {
  ...
  let prototype = {
    emit: function (event, ...args) {
      ...
      // Record changes when recording is enabled
      if (isRecording && event === "changed") {
        const change = {
          timestamp: new Date().toISOString(),
          module: name,
          key: args[0],
          value: cloneDeep(args[1]),
          fullState: cloneDeep(this)
        };
        userPresetHistory.push(change);
      }
    },
...
// Functions to manage user preset history
export function startRecording() {
  isRecording = true;
  userPresetHistory.length = 0; // Clear previous history
  console.log("Started recording user selections");
}

export function stopRecording() {
  isRecording = false;
  console.log("Stopped recording user selections");
}

export function getUserPresetHistory() {
  return cloneDeep(userPresetHistory);
}
